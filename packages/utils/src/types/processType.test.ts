...
`
  ...
  test('enum declarations', () => {
    ...
    const types = processType(enumDeclaration.getType(), enumDeclaration)
  ...
  test('class declarations', () => {
    ...
    const types = processType(classDeclaration.getType(), classDeclaration)
  ...
  test('accepts mixed types', () => {
    ...
    nodes
      .map((node) => processType(node.getType(), node))
      .forEach((doc) => {
        if (doc?.kind === 'Class') {
          doc.accessors
  ...
  test('unknown initializers', () => {
    ...
    const counterTypes = processType(counterVariable.getType(), counterVariable)
    const promiseVariable = sourceFile.getVariableDeclarationOrThrow('promise')
    const promiseTypes = processType(promiseVariable.getType(), promiseVariable)
    const awaitedVariable = sourceFile.getVariableDeclarationOrThrow('awaited')
    const awaitedTypes = processType(awaitedVariable.getType(), awaitedVariable)
